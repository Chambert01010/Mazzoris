...
from statements.utils import StatementProcessingError, extract_inbursa_data
...
def main(pdf_path: str) -> int:
    ...
    with source.open("rb") as handle:
        movimientos_df, resumen_df = extract_inbursa_data(handle)

    base_name = source.with_suffix("")
    xlsx_path = base_name.with_name(f"{base_name.name}_inbursa.xlsx")
    csv_path = base_name.with_name(f"{base_name.name}_inbursa.csv")
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Uso: py inbursaGemini2.0.py <ruta_pdf>")
        raise SystemExit(1)
